...
def emit_cpp_header(path: Path, data: bytes):
    with open(path, "w") as f:
        f.write("// Weights for a GRU-based German grapheme-to-phoneme\n"
        ...
        f.write("#pragma once\n#include <cstddef>\n#include <cstdint>\n\n")
        f.write("inline const unsigned char G2P_NEURAL_MODEL_WEIGHTS_DE[] =\n")
        chunk = 4000
        for i in range(0, len(data), chunk):
            f.write(f'"{escape_c_string(data[i:i + chunk])}"\n')
        f.write(";\n")
        f.write(f"inline const size_t G2P_NEURAL_MODEL_WEIGHTS_DE_LEN = {len(data)};\n")
...
def main():
    ...
    ap.add_argument("--output-header", type=Path,
                    default=HERE.parent.parent / "src" / "TinyTTSTools" / "Data" / "neural" / "G2PNeuralWeightsDE_data.h")
    args = ap.parse_args()
    ...
    data_out = data_dir / "g2p_model_de.bin"
