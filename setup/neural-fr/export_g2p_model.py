...
def emit_cpp_header(path: Path, data: bytes):
    with open(path, "w") as f:
        f.write("// Weights for a GRU-based French grapheme-to-phoneme\n"
                "// fallback -- trained by setup/neural-fr/train_g2p_model.py,\n"
                "// exported by setup/neural-fr/export_g2p_model.py. See\n"
                "// setup/neural-fr/README.md (or docs/SETUP.md) for how to retrain.\n"
                "// NOT wired into G2PNeuralModel.h yet -- its PHONEME_TABLE is\n"
                "// English-specific; a French-language model needs its own\n"
        ...
        f.write("#pragma once\n#include <cstddef>\n#include <cstdint>\n\n")
        f.write("inline const unsigned char G2P_NEURAL_MODEL_WEIGHTS_FR[] =\n")
        chunk = 4000
        for i in range(0, len(data), chunk):
            f.write(f'"{escape_c_string(data[i:i + chunk])}"\n')
        f.write(";\n")
        f.write(f"inline const size_t G2P_NEURAL_MODEL_WEIGHTS_FR_LEN = {len(data)};\n")
...
def main():
    ...
    ap.add_argument("--output-header", type=Path,
                    default=HERE.parent.parent / "src" / "TinyTTSTools" / "Data" / "neural" / "G2PNeuralWeightsFR_data.h")
    args = ap.parse_args()
    ...
    data_out = data_dir / "g2p_model_fr.bin"
