 ...
 * can never be exact; it's a longest-match rule engine (see
 * G2PRuleBasedModelBase for the shared word-start/word-end/digraph/
 * single-letter scaffolding -- word-end suffixes, then trigraphs/digraphs,
 * then single letters) covering the common vowel digraphs, silent
 * letters, doubled consonants and suffixes that a naive per-letter
 * mapping misses. Stress is not modeled (no digits are ever emitted) --
 * callers needing precise stress should prefer a dictionary lookup and
 * treat this purely as a last resort for out-of-vocabulary words.
 *
 ...
 */
class G2PRuleBasedModelEN : public G2PRuleBasedModelBase {
 protected:
